...
# Meterle algo más- gráficos sencillos para que vean que nos lo curramos ¿?
...
##################################################################################
#3.2) Exploración gráfica inicial - dejar esta exploración para el final del preprocesado
##################################################################################
...
# - 8 )   Dicotomizar precipitaciones
# - 9 )   Dicotomizar precipitaciones
...
#### 8 ) Sustituir PRESMAX y PRESMIN por una columna con su rango
...
RemoveOutliers2 <- function(dataset,col_outliers) {

  a = dataset[-col_outliers,]
  result <- a
}
...
#A MEJORAR: Intentar usar apply lapply para poder hacer esto con todas las variables y simplificar
...
O_8 = InciOutliers(datos_galicia,outliers_ALTITUD)
...
O_12 = InciOutliers(datos_galicia,outliers_VELMEDIA)

outliers_RACHA = FindOutliers(datos_galicia$RACHA)
O_13 = InciOutliers(datos_galicia,outliers_RACHA)

outliers_SOL = FindOutliers(datos_galicia$SOL)
O_14 = InciOutliers(datos_galicia,outliers_SOL)

outliers_PRES_RANGE = FindOutliers(datos_galicia$PRES_RANGE)
O_15 = InciOutliers(datos_galicia,outliers_PRES_RANGE)

length(names(numericas_input))
variable <- c(names(numericas_input))
incidencia <- c(O_1,O_2,O_3,O_4,O_5,O_6,O_7,O_8,O_9,O_10,O_11,O_12,O_13,O_14,O_15)
...
# Evidentemente una parte significativa de las variables necesita algún tratamiento para mitigar el efecto de
# los valores extremos en las observaciones --- seguir explicación
...
# categorizarla ...... seguir explicación
...
length(names(numericas_input))
variable <- c(names(numericas_input))
incidencia <- c(O_1,O_2,O_3,O_4,O_5,O_6,O_7,O_9,O_10,O_11,O_12,O_13,O_14,O_15)
...
dim(datos_galicia) # 14856


# 11 Categorizar velocidad media

# Hemos decidio también categorizar la velocidad media del viento, porque es una variable que presenta un porcentaje
# elevado de outliers --- seguir explicación
...
variable <- c(names(numericas_input))
incidencia <- c(O_1,O_2,O_3,O_4,O_5,O_6,O_7,O_9,O_10,O_11,O_13,O_14,O_15)
...
dim(datos_galicia) # 14855 


datos_galicia = RemoveOutliers(datos_galicia,datos_galicia$PRES_RANGE) 
dim(datos_galicia) #14182 # 8504 con perdidas eliminadas
...
dim(datos_galicia) # 13658 # 8107 con perdidas eliminadas


summary(numericas_input)
...
# Al igual que en el tratamiento posterior aplicable a los valores perdidos, podríamos haber 
#optado entre dos accionespodremos tomar dos acciones: la eliminación de los valores extremos o el 
# reemplazo por la media o la mediana. Hemos evitado llevar a cabo esta segunda posibilidad dado que
# la imputación de valores, dadas las características de nuestro dataset, restaría mucho valor pese a
# preservar el número de las observaciones y unos boxplot "bonitos".

# De todas formas, en el script vamos a mantener los códigos por si el tratamiento hubiera sido distinto.
...
#Como son categóricas, podríamos reemplazar por el valor más repetido, pero no sería correcto porque son bastantes
# observaciones y se distorsionaría la realidad

# Comprobamos que se han eliminado correctamente los valores perdidos:
...
# Añadir una sección de correlación con gráficos o algo para completar ¿?
