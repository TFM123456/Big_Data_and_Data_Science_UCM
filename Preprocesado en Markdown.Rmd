...
input<-as.data.frame(input[,-c(7,15)]) # Eliminamos pérdidas - variable objetivo continua
...
listaGraf <- dfplot_box(datos_galicia) 
boxplot_numericas = dfplot_box(numericas_input)

```


```{r boxplot01, echo=FALSE, warning=FALSE, message=FALSE}
gridExtra::marrangeGrob(boxplot_numericas, nrow = 3, ncol = 3)
...
input<-as.data.frame(datos_galicia[,-(1)])
input<-as.data.frame(input[,-c(7,15)]) 
row.names(input)<-datos_galicia$id
numericas_input <- input %>% select(is.numeric)
